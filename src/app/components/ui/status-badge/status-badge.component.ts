...
export type StatusBadgeVariant = 'active' | 'success' | 'warning' | 'pending' | 'archived' | 'complete' | 'reserved' | 'awarded';
...
export class StatusBadgeComponent {
  ...
  get sizeClasses(): string {
    ...
    const sizeMap: {[key: string]: string} = {
      'xs': `h-4 px-2 text-xs leading-4 ${widthPrefix}20`,
      'sm': `h-5 px-3 text-xs leading-5 ${widthPrefix}24`,
