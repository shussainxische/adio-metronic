...
export class InputComponent implements OnInit, OnChanges, ControlValueAccessor {
  ...
  private validationPatterns = {
    ...
    name: /^[a-zA-Z0-9\s]*$/,
    number: /^[0-9]*$/,
    decimal: /^[0-9]*\.?[0-9]*$/, // Allows decimal numbers
    arabic: /^[\u0600-\u06FF0-9\s]*$/,
  };
  ...
  private inputRestrictionPatterns = {
    ...
    name: /[a-zA-Z0-9\s]/,
    number: /[0-9]/,
    decimal: /[0-9.]/, // Allows digits and decimal point
    arabic: /[\u0600-\u06FF0-9\s]/,
  };
  ...
  get inputPattern(): string {
    ...
    switch (this.validationType) {
      ...
      case 'number':
        return '[0-9]*';
      case 'decimal':
        return '[0-9]*\\.?[0-9]*'; // Pattern for decimal numbers
      case 'arabic':
        return '[\\u0600-\\u06FF0-9\\s]*';
      default:
        return '';
  ...
  private applyDecimalRestrictions(value: string): string {
  ...
  private applyInputRestrictions(value: string): string {
    ...
    if (this.validationType === 'decimal') {
      return this.applyDecimalRestrictions(value);
    }
  ...
  onKeyPress(event: KeyboardEvent) {
    ...
    } else if (this.validationType === 'mobile') {
    ...
    if (this.getControl()) {
      ...
      if (this.minLength) {
        if (finalValue.length < this.minLength && finalValue.length > 0) {
          ...
          this.getControl().errors?.['minlength'] &&
          finalValue.length >= this.minLength
        ) {
